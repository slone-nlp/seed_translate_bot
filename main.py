...
def send_text_to_user(user_id, text, reply_markup=None, suggests=None, parse_mode='markdown'):
    if reply_markup is None:
        reply_markup = render_markup(suggests or [])
    logger.info("Response is:" + text)
    result = bot.send_message(user_id, text, reply_markup=reply_markup, parse_mode=parse_mode)
...
def process_message(msg: telebot.types.Message):
    ...
    if not text:
    ...
    elif text in {"/task"} or user.state_id == States.SUGGEST_TASK and text in {texts.RESP_SKIP_TASK}:
        # TODO: check if there is an unfinished current task, and deal with it properly
        task = DB.get_new_task(user=user)
        if task is None:
            send_text_to_user(user_id, 'Сейчас для вас нет никаких задач! Попробуйте зайти позже, проверить чужие переводы.', reply_markup=default_markup)
        else:
            ...
            DB.save_user(user)
            send_text_to_user(user_id, resp, suggests=suggests, parse_mode="html") # For some reason, markdow is malformed with urls
    elif user.state_id == States.SUGGEST_TASK and text in {texts.RESP_TAKE_TASK}:
        task = DB.get_task(user.curr_task_id)
        if task is None:
            send_text_to_user(user_id, "Простите, задача потерялась.", reply_markup=default_markup)
        ...
        resp, suggests = tasking.do_save_xsts_and_ask_for_translation(user=user, db=DB, user_text=text)
        DB.save_user(user)
        send_text_to_user(user_id, resp, suggests=suggests)

    # the lowest priority: accepting any text as translation!
    elif user.state_id == States.ASK_TRANSLATION:
        resp, suggests = tasking.do_save_translation_and_ask_for_next(user=user, db=DB, user_text=text)
        DB.save_user(user)
        send_text_to_user(user_id, resp, suggests=suggests)
    else:
        send_text_to_user(
